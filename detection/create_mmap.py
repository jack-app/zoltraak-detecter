...
def process_directory(model_path: str, file_path: str, mmap_path: str, conf_threshold: float = 0.1, target_bgr: tuple[int, int, int] = (0, 0, 255)):
	"""
...
if __name__ == "__main__":
	COLOR_MMAP_PATH = os.getenv("COLOR_MMAP_PATH", "") # 杖の先端部の色を保存するmmapファイルの場所を指定します。
	INPUT_IMAGE_PATH = os.getenv("INPUT_IMAGE_PATH", "") # 入力する画像のパスを指定します。
	POSITION_MMAP_PATH = os.getenv("POSITION_MMAP_PATH", "") # 出力先のmmapファイルの場所を指定します。
